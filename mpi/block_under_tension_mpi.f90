module Algorithms
    ...
    subroutine calculate_coordinates(ndivx, ndivy, ndivz, dx, width, thick, length, coord, alflag, nnum)
        integer, intent(in) :: ndivx, ndivy, ndivz
        double precision, intent(in) :: dx, width, thick, length
        double precision, dimension(:, :), intent(out) :: coord
        integer, dimension(:, :), intent(out) :: alflag
    ...
    subroutine calculate_boundary_points(ndivz, ndivy, nbnd, dx, width, thick, coord, nnum)
        integer, intent(in) :: ndivz, ndivy, nbnd
        double precision, intent(in) :: dx, width, thick
        double precision, intent(inout) :: coord(:,:)
        integer, intent(inout) :: nnum
        integer :: i, j, k
        double precision :: coordx, coordy, coordz
        ...
        do i = 1, ndivz
            ...
            enddo
        enddo
    end subroutine calculate_boundary_points
    
    
    subroutine calculate_disp_stendens(direction, totnode, coord, disp, stendens, numfam, pointfam, nodefam, delta, radij, bc, vol, fncst, sedload1)
        implicit none
        integer, intent(in) :: totnode, direction
        double precision, intent(in) :: coord(totnode,3), delta, radij, bc, vol, sedload1
        integer, intent(in) :: numfam(totnode,1), pointfam(totnode,1), nodefam(:,:)
        double precision, intent(inout) :: disp(totnode,3), stendens(totnode,3), fncst(totnode,3)
        integer :: i, j, cnode
        double precision :: idist, nlength, fac
        ...
        do i = 1, totnode
            ...
            do j = 1, numfam(i,1)
                ...
                stendens(i,direction) = stendens(i,direction) + 0.5d0 * 0.5d0 * bc * ((nlength - idist) / idist)**2 * idist * vol * fac
            enddo
            fncst(i,direction) = sedload1 / stendens(i,direction)
        enddo
    end subroutine calculate_disp_stendens
    
    subroutine calculate_cn(totint, velhalfold, disp, pforce, massvec, pforceold, dt, cn1, cn2)
        implicit none
        integer, intent(in) :: totint
        double precision, intent(in) :: velhalfold(totint,3), disp(totint,3), pforce(totint,3), massvec(totint,3), pforceold(totint,3), dt
        double precision, intent(inout) :: cn1, cn2
        integer :: i
...
integer ndivx, ndivy, ndivz, nnum, totnode, nt, maxfam, cnode, tt, nbnd, i, j, k
...
real*8, dimension(:,:), allocatable :: pforce, coord, pforceold, bforce, stendens, fncst, fncstold, disp, vel, velhalfold, velhalf, andisp, acc
integer, dimension(:,:), allocatable :: numfam, pointfam, nodefam, alflag
real*8, dimension(:,:), allocatable :: massvec

integer :: ierr, rank, size, start, end
...
start = rank * totint / size + 1
end = (rank + 1) * totint / size

allocate(pforce(end - start + 1, 3))
allocate(coord(end - start + 1, 3))
allocate(numfam(end - start + 1, 1))
allocate(pointfam(end - start + 1, 1))
allocate(nodefam(10000000, 1))
allocate(alflag(end - start + 1, 1))
allocate(pforceold(end - start + 1, 3))
allocate(bforce(end - start + 1, 3))
allocate(stendens(end - start + 1, 3))
allocate(fncst(end - start + 1, 3))
allocate(fncstold(end - start + 1, 3))
allocate(disp(end - start + 1, 3))
allocate(vel(end - start + 1, 3))
allocate(velhalfold(end - start + 1, 3))
allocate(velhalf(end - start + 1, 3))
allocate(acc(end - start + 1, 3))
allocate(massvec(end - start + 1, 3))
allocate(andisp(end - start + 1, 3))
...
call calculate_coordinates(ndivx, ndivy, ndivz, dx, width, thick, length, coord, alflag, nnum)
write(*,*) 'nnum = ', nnum
totint = nnum

!Material points of the boundary region - left
call calculate_boundary_points(ndivz, ndivy, nbnd, dx, width, thick, coord, nnum)
...
call calculate_disp_stendens(1, totnode, coord, disp, stendens, numfam, pointfam, nodefam, delta, radij, bc, vol, fncst, sedload1)
call calculate_disp_stendens(2, totnode, coord, disp, stendens, numfam, pointfam, nodefam, delta, radij, bc, vol, fncst, sedload1)
call calculate_disp_stendens(3, totnode, coord, disp, stendens, numfam, pointfam, nodefam, delta, radij, bc, vol, fncst, sedload1)
...
!Time integration
do tt = 1,nt
    if (rank == 0) write(*,*) 'tt = ', tt

    do i = 1,totint
        pforce(i - start + 1, 1:3) = 0.0d0
    ...
    call calculate_cn(totint, velhalfold, disp, pforce, massvec, pforceold, dt, cn1, cn2)
