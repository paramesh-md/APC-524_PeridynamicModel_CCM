module Algorithms_vib
    implicit none

    contains

    subroutine calculate_families(totnode, pointfam, numfam, coord, delta, nodefam)
        implicit none
        integer, intent(in) :: totnode
        real*8, intent(inout) :: coord(totnode, 3), delta
        integer, intent(inout) :: pointfam(totnode, 1), numfam(totnode, 1), nodefam(10000000, 1)
        integer :: i, j
        real*8 :: idist
        ...
        do i = 1,totnode
            ...
            enddo
        enddo
    end subroutine calculate_families

    subroutine calculate_stendens(totnode, stendens, numfam, nodefam, pointfam, coord, disp, delta, radij, bc, vol, sedload1, fncst)
        implicit none
        integer, intent(in) :: totnode
        real*8, intent(inout) :: stendens(totnode, 1), coord(totnode, 3), disp(totnode, 3), delta, radij, bc, vol, sedload1, fncst(totnode, 1)
        integer, intent(inout) :: numfam(totnode, 1), nodefam(10000000, 1), pointfam(totnode, 1)
        integer :: i, j, cnode
        real*8 :: idist, nlength, fac
        ...
        enddo
    end subroutine calculate_stendens

end module Algorithms_vib
...
call calculate_families(totnode, pointfam, numfam, coord, delta, nodefam)
...
call calculate_stendens(totnode, stendens, numfam, nodefam, pointfam, coord, disp, delta, radij, bc, vol, sedload1, fncst)
...
do tt = 1,nt
    ...
    !of the bar
    do nrao = 0, ntotrao
       andisp(tt,1) = andisp(tt,1) + ((-1.0d0)**(nrao)) / ((2.0d0 * nrao + 1.0d0)**2) * dsin((2.0d0 * nrao + 1.0d0) * pi * coord(500,1) / 2.0d0) * dcos((2.0d0 * nrao + 1.0d0) * pi * cwave * ctime / 2.0d0)
    enddo
